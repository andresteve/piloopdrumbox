...
Keypad::Keypad(String *userKeymap, byte *row, byte *col, byte numRows, byte numCols) {
	rowPins = row;
	columnPins = col;
...
void Keypad::begin(String *userKeymap) {
    keymap = userKeymap;
}

// Returns a single key only. Retained for backwards compatibility.
String Keypad::getKey() {
	single_key = true;
...
bool Keypad::updateList() {
	...
	for (byte r=0; r<sizeKpd.rows; r++) {
		for (byte c=0; c<sizeKpd.columns; c++) {
			...
			String keyString = keymap[r * sizeKpd.columns + c];
			...
			if ((idx == -1) && button) {
				for (byte i=0; i<LIST_MAX; i++) {
					if (key[i].kstr==NO_KEY) {		// Find an empty slot or don't add key to list.
						key[i].kstr = keyString;
						key[i].kcode = keyCode;
...
// Search by Stringacter for a key in the list of active keys.
// Returns -1 if not found or the index into the list of active keys.
int Keypad::findInList (String keystr) {
	for (byte i=0; i<LIST_MAX; i++) {
		if (key[i].kstr == keystr) {
			return i;
		}
	}
	return -1;
...
// New in 2.0
String Keypad::waitForKey() {
	String waitKey = NO_KEY;
...
void Keypad::addEventListener(void (*listener)(String)){
...
void Keypad::addStatedEventListener(void (*listener)(String, KeyState)){
...
void Keypad::transitionTo(byte idx, KeyState nextState) {
	...
	// Calls keypadEventListener on any key that changes state.
  	...
  	if (keypadEventListener!=NULL)  {
			keypadEventListener(key[idx].kstr);
		}
		//call the event listener that contains the key state, if available
		if (keypadStatedEventListener!=NULL)
		{
			keypadStatedEventListener(key[idx].kstr, nextState);
		}
	}
}
